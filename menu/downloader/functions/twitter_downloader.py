# menu/downloader/functions/twitter_downloader.py
...
def twitter_downloader():
    clear()
    print_cyber_panel("Twitter Downloader", "Unduh media (gambar/video) dari Twitter")

    url_input = cyber_input("URL Twitter (contoh: https://x.com/.../status/...) atau ketik '00' untuk kembali")
    ...
    try:
        with console.status("[bold green]Mengambil informasi media...[/bold green]", spinner="dots"):
            ...
            api_endpoint = f"{base_url}/api/downloader/twitter"
        ...
        if result.get("status") and result.get("media"):
            ...
            console.print(f"\n[bold green]✓ Ditemukan {len(media_list)} media (tipe: {media_type}).[/bold green]")

            post_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            post_folder_name = f"twitter_post_{post_timestamp}"
            ...
            log_filename = f"twitter_download_log_{timestamp}.json"
